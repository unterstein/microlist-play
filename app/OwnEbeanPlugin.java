 ...
 */
public class OwnEbeanPlugin extends Plugin {

    private final Application application;

    public OwnEbeanPlugin(Application application) {
    ...
    public void onStart() {
        ...
        Configuration ebeanConf = Configuration.root().getConfig("ownebean");
        ...
        if (ebeanConf != null) {
            for (String key : ebeanConf.keys()) {
                ...
                // own code
                config.setEncryptKeyManager(new TodoEncryptKeyManager());
                // own code

                try {
                    config.setDataSource(new WrappingDatasource(DB.getDataSource(key)));
                } catch (Exception e) {
                    throw ebeanConf.reportError(key, e.getMessage(), e);
                }
                ...
                for (String load : toLoad) {
                    load = load.trim();
                    if (load.endsWith(".*")) {
                        classes.addAll(application.getTypesAnnotatedWith(load.substring(0, load.length() - 2),
                ...
                for (String clazz : classes) {
                    try {
                        ...
                        throw ebeanConf.reportError(key, "Cannot register class [" + clazz + "] in Ebean server", e);
                    }
                }
                ...
                if (!application.isProd()) {
                    boolean evolutionsEnabled = !"disabled".equals(application.configuration().getString(
                            "evolutionplugin"));
                    if (evolutionsEnabled) {
                        ...
                        if (evolutionScript != null) {
                            ...
                            if (!evolutions.exists()
                                    || Files.readFile(evolutions).startsWith("# --- Created by Ebean DDL")) {
    ...
    /**
     * <code>DataSource</code> wrapper to ensure that every retrieved connection
